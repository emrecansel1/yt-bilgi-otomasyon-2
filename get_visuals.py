...
def clean_text(text):
    ...
    # Ses/müzik/efekt satırlarını tamamen kaldır
    text = re.sub(
...
def get_topic():
    ...
    # İlk gerçek anlatım cümlesini bul
    for line in lines:
...
def get_scenes(text):
    ...
    # Çok az sahne çıktıysa cümlelere böl
    if len(scenes) < 10:
...
def clean_scene(scene):
    ...
    # İlk birkaç gereksiz kelimeyi temizle
    scene = re.sub(
...
def main():
    ...
    for i, scene in enumerate(
        ...
        # Ses efekti gibi sahneleri atla
        if (
            len(scene) < 40
            or
